...
6. 产品和评论分页
7. 图片压缩：前端 [compress.js](public/js/compress.js) 已覆盖新上传；老图用 [scripts/shrink-uploads.sh](scripts/shrink-uploads.sh) 批量瘦身过一次（2026-04-23，131→63MB）。**后端动态多尺寸 / WebP / srcset 仍未做**，等 .cn 切 CDN 时一起
...
### 2026-04-23 · 图片加载速度过渡优化

**背景**
- 大陆用户反馈图片加载慢 + 偶发 ❓ 图标。根因诊断:CF Tunnel 免费版走国际链路回源,~2% 请求 TCP 连接超时(`curl` 返 `000`),失败分布随机而非固定某几张
- 大陆备案走流程中,几天后切 `.cn` 域名 + 国内 CDN 才是治本方案。本次只做"扔了也不心疼"的过渡优化:**不动 DB 字段、不迁外部存储、不引入新框架依赖**

**代码层**
- [server.js:170-175](server.js#L170):`express.static('/uploads')` 加 `{ maxAge: '30d', immutable: true }`,源站主动发长缓存头。文件名带时间戳 (`Date.now()-hash.ext`),天然避免同路径冲突
- 前端 `<img>` 全量补懒加载(改 4 处):
  - 产品卡片(原本已有 lazy)补 `decoding="async"` [public/index.html:33](public/index.html#L33)
  - 详情页 hero 图 `fetchpriority="high" decoding="async"`(首屏关键,**不 lazy**)[public/app.js:317](public/app.js#L317)
  - 详情页附图四宫格 `loading="lazy" decoding="async"` [public/app.js:303](public/app.js#L303)
  - 评论图 `loading="lazy" decoding="async"` [public/index.html:54](public/index.html#L54)
- [public/style.css](public/style.css) 末尾追加骨架屏 shimmer:图未加载时父容器跑柔和高光动画,`<img>` onload 后自然覆盖。尊重 `prefers-reduced-motion` 用户

**uploads/ 一次性瘦身**
- 新增 [scripts/shrink-uploads.sh](scripts/shrink-uploads.sh):ImageMagick 原地缩图,`-auto-orient`(先按 EXIF 转像素) → `-resize '1600x>'` → `-quality 82` → `-strip`(去 EXIF metadata 省几十 KB + 顺便抹位置信息)
- 安全机制:运行前整体 `cp -a` 到 `/data/xiaofamous/backups/uploads-pre-shrink-2026-04-23/`;BACKUP_DIR 已存在则拒绝覆盖(幂等保护);convert 失败保留原图不动
- 首次执行结果:**64/65 张处理,131MB → 63MB (-51%)**,iPhone 4032 宽原图降幅 70-83%
- 依赖:系统级 `imagemagick`(`sudo apt install imagemagick` 已装)

**Cloudflare Dashboard(用户手动点,不进 git)**
- **Cache Rule**:URI Path starts with `/uploads/` → Edge TTL 1 month,Browser TTL 7 days
- **Tiered Cache**:Smart Tiered Cache 开启(边缘 miss 先去地区中心取,不直接捅回源)
- **Purge Everything**:做过一次,清掉压图前缓存进 CF 的大图

**效果(实测)**
| 指标 | 前 | 后 |
|---|---|---|
| 公网失败率(61 张 × 多轮) | 2% (5/244) | 0% (0/183) |
| CF 缓存状态 | `REVALIDATED`(每次回源验证) | `HIT`(30 天不回源) |
| uploads/ 总大小 | 131 MB | 63 MB |
| CF 边缘实际拉图总量(61 张) | ~131 MB | 56 MB |
| 单张 iPhone 原图 | 3-5 MB | 0.7-1.3 MB |

**没做(等 .cn 切换时一起做,见 [docs/OSS_MIGRATION_PLAN.md](docs/OSS_MIGRATION_PLAN.md))**
- 迁阿里云 OSS / 国内 CDN:`ali-oss` 依赖装了但 server.js 从未引用,备案下来再接
- 改 DB `products.image` / `products.images` 字段(现在仍全是相对 `/uploads/...`)
- 后端 `sharp` 动态多尺寸 / `/img?w=800&fmt=webp` 路由
- 前端 `srcset` 响应式图片
- 评论区上传的图也走 shrink 脚本(当前脚本只跑过一次现有 uploads/;新上传走前端 [compress.js](public/js/compress.js),绕过前端仍可能传大图)
- CF Polish / Auto WebP(Polish 是付费功能,且 .cn 后可能不再用 CF)

**风险 / 注意**
- **备份观察期**:`/data/xiaofamous/backups/uploads-pre-shrink-2026-04-23/`(132MB)保留一周,确认无人反馈图糊再删。回滚命令:
  ```bash
  rm -rf /home/andrew/xiaofamoustalk/uploads \
    && cp -a /data/xiaofamous/backups/uploads-pre-shrink-2026-04-23 /home/andrew/xiaofamoustalk/uploads
  ```
- **孤儿 + 损坏的 PNG**:`uploads/1776745819137-ea23b6e5.png`(2.3MB)文件底部 ~40% 像素数据被截断(`identify -format '%w'` 报 `Expected 8192 bytes; found 4914 bytes`),且 DB 无产品引用。压图脚本会跳过它;可手动 `rm` 清掉,不影响任何产品
- **CF Cache Rule TTL 是 30 天 immutable**:如果以后修改了 [server.js](server.js) 的文件名生成逻辑(现在是 `Date.now()-hash.ext`),出现同名覆盖时 CF 边缘不会感知,会继续返回旧图 30 天
- **压图脚本非幂等重跑**:第二次跑要先删备份目录或改日期后缀(BACKUP_DIR 存在就 refuse);实际上 `-strip` 已经去掉 EXIF,第二次再压也没啥油水
- **ali-oss 已在 [package.json](package.json) 依赖树但 server.js 从未 import**:切 OSS 时直接用现有 SDK,不需要再 `npm install`

**下一个接手 AI 优先做**
1. 等用户备案通过 + `.cn` 就绪 → 启动 [docs/OSS_MIGRATION_PLAN.md](docs/OSS_MIGRATION_PLAN.md) 迁移方案
2. 观察 `/data/xiaofamous/backups/uploads-pre-shrink-2026-04-23/` 过一周没问题 → 手动删
3. 清理孤儿损坏 PNG(可选)
